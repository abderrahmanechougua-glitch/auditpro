...
class Workspace(QWidget):
    ...
    def _build_ui(self):
        ...
        self.module_hint_banner = QLabel("")
        self.module_hint_banner.setObjectName("ModuleHintBanner")
        self.module_hint_banner.setWordWrap(True)
        self.module_hint_banner.setVisible(False)
        main_layout.addWidget(self.module_hint_banner)

        self.drop_feedback = QLabel("Déposez un fichier ici pour alimenter le premier champ compatible.")
        self.drop_feedback.setObjectName("DropFeedback")
        self.drop_feedback.setWordWrap(True)
    ...
    def set_output_dir_hint(self, output_dir: str):
        ...
        self._output_dir_hint = (output_dir or "").strip()

    @staticmethod
    def _is_float_number(value) -> bool:
        """Return True when value can represent a float with decimals."""
    ...
    def _build_inputs(self, inputs: list[ModuleInput]):
        ...
        for inp in inputs:
            ...
            if inp.input_type == "file":
                ...
                self.file_input_keys.append(inp.key)
                row += 1
    ...
    def _build_params(self, params: list[dict]):
        ...
        for i, p in enumerate(params, 1):
            ...
            if p.get("type") == "number":
                min_v = p.get("min", 0)
                max_v = p.get("max", 999999)
                default_v = p.get("default", 0)
                step_v = p.get("step", 1)

                use_float = any(
                    self._is_float_number(v)
                    for v in (min_v, max_v, default_v, step_v)
                )
                ...
                if use_float:
                    ...
                    w.setSingleStep(float(step_v))
                    w.setValue(float(default_v))
                else:
                    w = QSpinBox()
                    w.setRange(int(min_v), int(max_v))
                    w.setSingleStep(int(step_v) if int(step_v) > 0 else 1)
                    w.setValue(int(default_v))
    ...
    def _set_drop_feedback(self, message: str, visible: bool):
        self.drop_feedback.setText(message)
        self.drop_feedback.setVisible(visible)

    def _first_file_input_key(self) -> str:
        return self.file_input_keys[0] if self.file_input_keys else ""

    def _accept_dropped_file(self, path: str) -> bool:
        file_key = self._first_file_input_key()
        if not file_key:
            self._set_drop_feedback("Aucun champ fichier actif pour recevoir ce dépôt.", True)
            return False

        widget = self.input_widgets.get(file_key)
        if not isinstance(widget, QLineEdit):
            return False

        widget.setText(path)
        self.loaded_files[file_key] = path
        self._refresh_workspace_state()
        self.file_loaded.emit(path)
        self.file_dropped.emit(path)
        self._set_drop_feedback(f"Fichier déposé: {Path(path).name}", True)
        return True
    ...
    def _browse_file(self, line_edit: QLineEdit, key: str, extensions: str, multiple: bool = False):
        if multiple:
            ...
            if paths:
                ...
                self._refresh_workspace_state()
                self._set_drop_feedback(f"Fichier sélectionné: {Path(paths[0]).name}", True)
            ...
            if path:
                ...
                self._refresh_workspace_state()
                self._set_drop_feedback(f"Fichier sélectionné: {Path(path).name}", True)
                self.file_loaded.emit(path)
    ...
    def dragEnterEvent(self, event):
        ...
        if urls and urls[0].isLocalFile():
            ...
            self._set_drop_feedback("Relâchez pour charger le fichier dans le workflow actif.", True)
            return
        event.ignore()
    ...
    def dropEvent(self, event):
        ...
        if not path or not os.path.isfile(path):
            self._set_drop_feedback("Le dépôt ne contient pas un fichier local valide.", True)
            event.ignore()
            return

        if self._accept_dropped_file(path):
            event.acceptProposedAction()
            return

        self._set_drop_feedback("Impossible d'associer ce fichier à un champ du module actif.", True)
        event.ignore()
    ...
    def _on_preview(self):
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                self.module_hint_banner.setVisible(True)
                self._refresh_workspace_state()
            else:
                self._preview_loaded = False
                self._last_preview_rows = 0
    ...
    def show_result(self, result):
        ...
        if result.success:
            ...
            if not self._output_path and self._output_dir_hint:
                self._output_path = self._output_dir_hint
    ...
    def _on_open_output(self):
        """Ouvre le fichier résultat; dossier seulement en dernier recours."""
        ...
        # Si on a seulement un dossier (fallback), essayer d'ouvrir le fichier le plus récent.
        if p.is_dir():
            # Exclure les fichiers temporaires/verrous générés par Excel et Office.
            ignored_prefixes = ("~$", ".")
            ignored_suffixes = (".tmp", ".temp", ".part", ".lock", ".lck")
            candidates = [
                fp for fp in p.glob("*")
                if fp.is_file()
                and not fp.name.startswith(ignored_prefixes)
                and fp.suffix.lower() not in ignored_suffixes
            ]
            if candidates:
                target_path = max(candidates, key=lambda fp: fp.stat().st_mtime)
