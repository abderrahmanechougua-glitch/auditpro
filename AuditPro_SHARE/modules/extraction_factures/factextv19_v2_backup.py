...
def normalize_amount(amount_str: str) -> Optional[float]:
    ...
    s = re.sub(r"[^\d.\-]", "", s)
    # Nettoyer les séparateurs redondants en fin de valeur (ex: "1200.")
    s = s.strip(".")
...
# Montants OCR: autorise quelques lettres souvent confondues avec des chiffres
...
PATTERNS_HT = [
    # Avec label explicite
    (rf"Total\s+HT\s*[:\s]*{AMOUNT_RE}", 95),                         # Total HT 175 000,00
    (rf"Montant\s+(?:total\s+)?H\.?T\.?\s*[:\s]*{AMOUNT_RE}", 95),  # Montant total H.T 25 000,00
    (rf"Sous[- ]?total\s*[:\s]*(?:MAD\s*)?{AMOUNT_RE}", 90),          # Sous-total MAD 25,000.00
    (rf"H\.?T\.?\s*[:\s]+{AMOUNT_RE}", 80),
    # BEST PROFIL: dans le tableau en bas
    (rf"Montant\s+H\.T\.\s*\n?\s*{AMOUNT_RE}", 85),
    # Avec MAD devant
    (rf"(?:MAD|DH)\s+{AMOUNT_RE}\s*$", 75),
]

PATTERNS_TVA_MONTANT = [
    # TVA montant avec label
    (rf"T\.?V\.?A\.?\s*(?:\(\s*\d+\s*%?\s*\))?\s*[:\s]*(?:MAD\s*)?{AMOUNT_RE}", 90),
    (rf"TVA\s+\d+\s*%?\s*[:\s]*{AMOUNT_RE}", 90),                    # TVA 20 % 35 000,00
    (rf"T\.V\.A\.\s*[:\s]*{AMOUNT_RE}", 85),
    (rf"TVA\s*[:\s]+{AMOUNT_RE}", 80),
]
...
PATTERNS_TTC = [
    ...
    # Labels explicites
    (rf"Total\s+G[ée]n[ée]ral\s*[:\s]*{AMOUNT_RE}", 95),             # AFRIQUIA: Total Général
    (rf"Total\s+T\.?T\.?C\.?\s*[:\s]*{AMOUNT_RE}", 95),            # Total TTC
    (rf"Montant\s+(?:total\s+)?T\.?T\.?C\.?\s*[:\s]*{AMOUNT_RE}", 95),  # Montant total T.T.C
    (rf"Net\s+[àa]\s+payer\s*[:\s]*(?:MAD\s*)?{AMOUNT_RE}", 90),
    # Montant suivi de DH/MAD
    (rf"{AMOUNT_RE}\s*(?:DH|DIRHAMS?|MAD)\s*$", 75),
    # MAD devant le montant
    (rf"MAD\s+{AMOUNT_RE}\s*$", 75),
]
